...
fn rc_ensure_unique<A: Clone, D: Dimension>(rcarr: &mut ArrayBase<OwnedRcRepr<A>, D>) {
    if Rc::get_mut(&mut rcarr.data.0).is_some() {
        return;
    }
    if rcarr.dim.size() <= rcarr.data.0.len() / 2 {
        // Create a new vec if the current view is less than half of
        // backing data.
        unsafe {
            *rcarr = ArrayBase::from_shape_vec_unchecked(rcarr.dim.clone(),
                                                         rcarr.iter()
                                                         .cloned()
                                                         .collect());
        }
    ...
    let rcvec = &mut rcarr.data.0;
    let a_size = mem::size_of::<A>() as isize;
    let our_off = if a_size != 0 {
        (rcarr.ptr as isize - rcvec.as_ptr() as isize) / a_size
    } else { 0 };
    let rvec = Rc::make_mut(rcvec);
    unsafe {
        rcarr.ptr = rvec.as_mut_ptr().offset(our_off);
    }
}
